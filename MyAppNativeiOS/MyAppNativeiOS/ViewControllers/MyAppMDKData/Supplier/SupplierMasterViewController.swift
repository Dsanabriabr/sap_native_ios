...
class SupplierMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    var loadEntitiesBlock: (() async throws -> [MyAppMDKDataFmwk.Supplier])?
    private var entities: [MyAppMDKDataFmwk.Supplier] = .init()
    ...
    private func addTableHeaderView() {
        ...
        tableHeader.headlineText = "Supplier"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        return CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "SupplierID", value: "\(supplier.supplierID!)")
    }
    ...
    func requestEntities() async throws {
        entities = try await loadEntitiesBlock!().sorted(by: { ($0.supplierID!) < ($1.supplierID!) })
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            detailViewController.navigationItem.title = "\(entities[(tableView.indexPathForSelectedRow?.row)!].supplierID != nil ? "\(entities[(tableView.indexPathForSelectedRow?.row)!].supplierID!)" : "")"
...
extension SupplierMasterViewController: MyAppMDKDataEntitySetUpdaterDelegate {
