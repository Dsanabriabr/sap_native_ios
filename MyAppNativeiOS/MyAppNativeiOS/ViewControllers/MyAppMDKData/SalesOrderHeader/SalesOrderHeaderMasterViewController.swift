...
class SalesOrderHeaderMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    var loadEntitiesBlock: (() async throws -> [MyAppMDKDataFmwk.SalesOrderHeader])?
    private var entities: [MyAppMDKDataFmwk.SalesOrderHeader] = .init()
    ...
    private func addTableHeaderView() {
        ...
        tableHeader.headlineText = "SalesOrderHeader"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        return CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "SalesOrderID", value: "\(salesOrderHeader.salesOrderID!)")
    }
    ...
    func requestEntities() async throws {
        entities = try await loadEntitiesBlock!().sorted(by: { ($0.salesOrderID!) < ($1.salesOrderID!) })
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            detailViewController.navigationItem.title = "\(entities[(tableView.indexPathForSelectedRow?.row)!].salesOrderID != nil ? "\(entities[(tableView.indexPathForSelectedRow?.row)!].salesOrderID!)" : "")"
...
extension SalesOrderHeaderMasterViewController: MyAppMDKDataEntitySetUpdaterDelegate {
