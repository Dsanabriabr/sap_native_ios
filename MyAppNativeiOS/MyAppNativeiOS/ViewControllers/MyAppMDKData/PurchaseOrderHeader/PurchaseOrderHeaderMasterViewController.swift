...
class PurchaseOrderHeaderMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    var loadEntitiesBlock: (() async throws -> [MyAppMDKDataFmwk.PurchaseOrderHeader])?
    private var entities: [MyAppMDKDataFmwk.PurchaseOrderHeader] = .init()
    ...
    private func addTableHeaderView() {
        ...
        tableHeader.headlineText = "PurchaseOrderHeader"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        return CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "PurchaseOrderID", value: "\(purchaseOrderHeader.purchaseOrderID!)")
    }
    ...
    func requestEntities() async throws {
        entities = try await loadEntitiesBlock!().sorted(by: { ($0.purchaseOrderID!) < ($1.purchaseOrderID!) })
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            detailViewController.navigationItem.title = "\(entities[(tableView.indexPathForSelectedRow?.row)!].purchaseOrderID != nil ? "\(entities[(tableView.indexPathForSelectedRow?.row)!].purchaseOrderID!)" : "")"
...
extension PurchaseOrderHeaderMasterViewController: MyAppMDKDataEntitySetUpdaterDelegate {
