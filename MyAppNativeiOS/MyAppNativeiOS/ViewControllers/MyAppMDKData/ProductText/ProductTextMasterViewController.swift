...
class ProductTextMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    var loadEntitiesBlock: (() async throws -> [MyAppMDKDataFmwk.ProductText])?
    private var entities: [MyAppMDKDataFmwk.ProductText] = .init()
    ...
    private func addTableHeaderView() {
        ...
        tableHeader.headlineText = "ProductText"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        return CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ID", value: "\(productText.keyID!)")
    }
    ...
    func requestEntities() async throws {
        entities = try await loadEntitiesBlock!().sorted(by: { ($0.keyID!) < ($1.keyID!) })
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            detailViewController.navigationItem.title = "\(entities[(tableView.indexPathForSelectedRow?.row)!].keyID != nil ? "\(entities[(tableView.indexPathForSelectedRow?.row)!].keyID!)" : "")"
...
extension ProductTextMasterViewController: MyAppMDKDataEntitySetUpdaterDelegate {
